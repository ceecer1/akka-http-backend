...
final class SimpleLruCache[V](val maxCapacity: Int, val initialCapacity: Int) extends Cache[V] {
  ...
  private[util] val store = new ConcurrentLinkedHashMap.Builder[Any, Future[V]]
...
final class ExpiringLruCache[V](maxCapacity: Long, initialCapacity: Int,
  ...
  private[util] val store = new ConcurrentLinkedHashMap.Builder[Any, Entry[V]]
    .initialCapacity(initialCapacity)
    .maximumWeightedCapacity(maxCapacity)
    .build()
...
private[util] class Entry[T](val promise: Promise[T]) {
